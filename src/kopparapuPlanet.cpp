...
//extract and calculate the planet parameters
  ...
  double lbol = pow(10.0,-0.4*(Lenses->data[ln][MBOL]-4.75));
  double seff_in = hz_seff(Lenses->data[ln][TEFF], kopparapu_moist_greenhouse);
  double seff_out = hz_seff(Lenses->data[ln][TEFF], 
			    kopparapu_maximum_greenhouse);

  double hzmultiplier=log10(Event->params[AA]);

  double logamin = log10(sqrt(lbol/seff_in)) + hzmultiplier;
  double logamax = log10(sqrt(lbol/seff_out)) + hzmultiplier;

  double u0max=1;

  //Semimajor axis is currently holding the a/aHZ position - swap them...
  Event->params[AA] = pow(10,logamin + ran2(Paramfile->seed) * (logamax-logamin));
