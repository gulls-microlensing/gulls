...
double VBMicrolensing::BinaryMag0(double a1, double q1, double y1v, double y2v, _sols** Images) {
	static VBMcomplex a, q, m1, m2, y;
	static double av = -1.0, qv = -1.0;
	static VBMcomplex  coefs[24], d1, d2, dy, dJ, dz;
...
double VBMicrolensing::BinaryMag(double a1, double q1, double y1v, double y2v, double RSv, double Tol, _sols** Images) {
	static VBMcomplex a, q, m1, m2, y0, y, yc, z, zc;
	static double av = -1.0, qv = -1.0;
	static VBMcomplex coefs[24], d1, d2, dy, dJ, dz;
	...
	y = y0 + VBMcomplex(RSv * cos(thoff), RSv * sin(thoff));
	...
	while (flag == 0) {
		...
		if (Prov->length > 0) {
			...
			y = y0 + VBMcomplex(RSv * cos(stheta->th), RSv * sin(stheta->th));
		}
	}
#ifdef _PRINT_TIMES
	...
	do {
		stheta = Thetas->insert(th);
		y = y0 + VBMcomplex(RSv * cos(th), RSv * sin(th));
#ifdef _PRINT_TIMES
		tim0 = Environment::TickCount;
#endif
		//if (NPS == 44) {
...
#define _Jacobians2\
	dy = VBMcomplex(-sin(theta->th), cos(theta->th))*coefs[23];\
...
_curve* VBMicrolensing::NewImages(VBMcomplex yi, VBMcomplex * coefs, _theta * theta) {
	static VBMcomplex  y, yc, z, zc, J1, J1c, dy, dz, dJ, J2, J3, dza, za2, zb2, zaltc, Jalt, Jaltc, JJalt2;
	static VBMcomplex zr[5] = { 0.,0.,0.,0.,0. };
...
// s: array of size n-1 with the separations in VBMcomplex coordinates from the first mass.
////////// What does it make?
...
void VBMicrolensing::SetLensGeometry(int nn, double* pr) {
	...
	VBMcomplex* s = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nn));

	for (int i = 0, j = 2; i < nn; ++i, j += 3) {
		q[i] = pr[j];
	}
	for (int i = 0, j = 0; i < nn; ++i, j += 3) {
		s[i] = VBMcomplex(pr[j], pr[j + 1]);
	}
...
void VBMicrolensing::SetLensGeometry(int nn, double* q, VBMcomplex * s) {
...
void VBMicrolensing::SetLensGeometry_spnp(int nn, double* q, VBMcomplex * s) {
	static double sumq, qmin, Jac;
	static int iqmin, dg;
	static VBMcomplex pbin[2], z, S2, fac;
	static int i, j;
...
void VBMicrolensing::SetLensGeometry_multipoly(int nn, double* q, VBMcomplex * s) {
	static int j, i, x, k, p, dg;
	static double tempq, sumq;
	static VBMcomplex temps, pbin[2];
...
double VBMicrolensing::MultiMag0(VBMcomplex yi, _sols * *Images) {
...
double VBMicrolensing::MultiMag0(VBMcomplex y) {
	static _sols* images;
	static double mag;
...
double VBMicrolensing::MultiMag0(double y1, double y2) {
	static _sols* images;
	static double mag;
	VBMcomplex y = VBMcomplex(y1, y2);
	mag = MultiMag0(y, &images);
	delete images;
	return mag;
}

double VBMicrolensing::MultiMag(VBMcomplex yi, double RSv, double Tol, _sols * *Images) {
	static VBMcomplex y0;
	...
	try {
		...
		stheta = Thetas->insert(th);
		stheta->maxerr = 1.e100;
		y = y0 + VBMcomplex(RSv * cos(thoff), RSv * sin(thoff)); // first image
		...
		if (squarecheck) {
			...
			while (itheta->next) {
				...
				y = y0 + VBMcomplex(rhorad2 * cos(th), rhorad2 * sin(th));
...
double VBMicrolensing::MultiMag(VBMcomplex y, double RSv, double Tol) {
	static _sols* images;
	static double mag;
...
double VBMicrolensing::MultiMag(VBMcomplex y, double RSv) {
	static _sols* images;
	static double mag;
	mag = MultiMag(y, RSv, Tol, &images);
	delete images;
...
double VBMicrolensing::MultiMag(double y1, double y2, double RSv) {
	static _sols* images;
	static double mag;
	mag = MultiMag(VBMcomplex(y1, y2), RSv, Tol, &images);
	delete images;
	return mag;
}
...
#define _MJacobiansgood\
	dy = VBMcomplex(-sin(theta->th)*rho, cos(theta->th)*rho);\
...
void VBMicrolensing::initroot() {
	static VBMcomplex fac, fac2, z, S2;
	static double Jac;
	...
	_Jac
		while (Jac <= 0) {	// Check if main image has positive parity. Otherwise displace
			z = z + ((abs2(y) > 1.e-6) ? y : VBMcomplex(0, 1));
			_Jac
		}
	init[n] = z;
...
int VBMicrolensing::froot(VBMcomplex zi) {
	static VBMcomplex z, zc, S1, S2, S2v, S3, zo, zo2, epso, epsbase, epsn, epsl, gradL, zl, fac, fac2, dz, dzo, TJold, TJnew, Lv, den;
			...
			_Jac
				...
				if (signbit(Jac) != signbit(Jacold)) {
					...
					if (abs2(epsn + dz) < 0.25 * abs2(epsn) || fad < 2) {
						dz = VBMcomplex(0.33, 0.01) * dz * fad;
						flagmain = 3;
...
_curve* VBMicrolensing::NewImages(_theta * theta) {
	...
	static VBMcomplex z, zc, dy, dz, J2, J3, Jalt, JJalt2, Jaltc, J1c2;
	static VBMcomplex S2, S2c, S3, S3c, vec, newseed0;
	static double ob2, dJ2, cq, Jac, imul, phi;
	static VBMcomplex newseedtrial[6] = { VBMcomplex(1,0.5),VBMcomplex(1,-0.5), 0.5, 0.75, 2., 4., };
	...
	while (ngood > ngoodold) {
		...
		for (int i = ngoodold; i < ngood; i++) {
			vec = VBMcomplex(2, 1 + sqrt(errs[i]) / abs(grads[i]));
	...
	while (nminus - nplus + 1 != n) {
		...
		z = imul * sqrt(m[imass]) * VBMcomplex(cos(phi), sin(phi)) + a[imass];
		froot(z);
		checkroot(theta);
		if (ngood > ngoodold) {
			if (Jacs[ngoodold] > 0) {
...
void VBMicrolensing::initrootpoly() {
	static double mrt;
	static VBMcomplex dev, dev2, zplus, shear, alpha0;
	static int ir;
	zplus = y;
	ir = nroots - 1;
	for (int i = 0; i < n; i++) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < 2 * (n - 1); j++) {
			dev = M_PI * (j / (n - 1.) + 0.5);
			zr[ir] = a[i] + dev2 * VBMcomplex(cos(dev.re), sin(dev.re));
			ir--;
		}
	}
}
...
int VBMicrolensing::findimagepoly(int i) {
	static VBMcomplex z, zc, yc, LL, zo, delta, lambda;
...
_curve* VBMicrolensing::NewImagespoly(_theta * theta) {
	static VBMcomplex  yc, z, zc, zo, delta, dy, dz, J2, J3, Jalt, Jaltc, JJalt2, LL, J1c2, dzita;
...
void VBMicrolensing::TripleLightCurve(double* pr, double* ts, double* mags, double* y1s, double* y2s, int np) {
	...
	s[1] = s[0] * q[0];
	s[0] = -q[1] * s[0];
	s[2] = exp(pr[7]) * VBMcomplex(cbeta, sbeta) + s[0];
	//	_sols *Images; double Mag; // For debugging
	...
	for (int i = 0; i < np; i++) {
		...
		if (mindi >= 10.) {
			...
			mags[i] = MultiMag(VBMcomplex(y1s[i], y2s[i]), rho, Tol);
		}
	}
}
...
void VBMicrolensing::LightCurve(double* pr, double* ts, double* mags, double* y1s, double* y2s, int np, int nl) {
	...
	VBMcomplex* s = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nl));
	...
	s[0] = VBMcomplex(0, pr[3]);
	for (int i = 1, j = 1; i < nl; ++i, j += 3) {
		s[i] = VBMcomplex(pr[j + 3], pr[j + 4]);
	}
...
double VBMicrolensing::TripleLightCurve(double* pr, double t) {
	...
	if (mindi >= 10.) {
		...
		return MultiMag(VBMcomplex(y_1, y_2), rho, Tol);
	}
}
...
_sols* VBMicrolensing::PlotCrit() {
	...
	for (int j = 0; j < NPcrit; j++) {
		ej = VBMcomplex(cos(2 * j * M_PI / NPcrit), -sin(2 * j * M_PI / NPcrit));
	...
	for (Prov = CriticalCurves->last; Prov; Prov = Prov->prev) {
		...
		for (_point* scanpoint = Prov->first; scanpoint; scanpoint = scanpoint->next) {
			y = z = VBMcomplex(scanpoint->x1 - s_offset->re, scanpoint->x2 - s_offset->im);
			for (int i = 0; i < n; i++) {
				y = y - m[i] / conj(z - a[i]);
			}
...
_sols* VBMicrolensing::PlotCrit(double a1, double q1) {
	...
	for (int j = 0; j < NPcrit; j++) {
		ej = VBMcomplex(cos(2 * j * M_PI / NPcrit), -sin(2 * j * M_PI / NPcrit));
		VBMcomplex  coefs[5] = { a * a / 16.0 * (4.0 - a * a * ej) * (1.0 + q),a * (q - 1.0),(q + 1.0) * (1.0 + a * a * ej / 2.0),0.0,-(1.0 + q) * ej };
	...
	for (Prov = CriticalCurves->last; Prov; Prov = Prov->prev) {
		Prov2 = new _curve;
		for (_point* scanpoint = Prov->first; scanpoint; scanpoint = scanpoint->next) {
			x1 = VBMcomplex(scanpoint->x1 - centeroffset, 0.0);
			x2 = VBMcomplex(scanpoint->x2, 0.0);
...
void VBMicrolensing::polyproduct(VBMcomplex * __restrict p1, int n1, VBMcomplex * __restrict p2, int n2, VBMcomplex * __restrict pdest) {
...
void VBMicrolensing::copypol(VBMcomplex * __restrict p1, int n1, VBMcomplex * __restrict pdest) {
	for (int i = 0; i <= n1; i++) {
...
void VBMicrolensing::change_n(int nn) {
	...
	nnm1 = n2 - n;
	nroots = 2 * n2 + 1;
	coefs = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots + 1));
	pmza = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	pmza2 = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	pyaza = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	ppmy = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	za = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	za2 = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	for (int i = 0; i < n; i++) {
		pmza[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
		pmza2[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (2 * n - 1));
		pyaza[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (n + 1));
		ppmy[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nnm1 + 1));
		za[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
		za2[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	}

	pza = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (n + 1));
	pza2 = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (2 * n + 1));
	pdum = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots + 1));
	ppy = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots + 1));

	zr = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	zcr = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (2 * n));
	for (int i = 0; i < nroots; i++) {
		zr[i] = 0;
	}
	for (int i = 0; i < 2 * n; i++) {
	...
	J1 = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	J1c = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	zaltc = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	worst = (int*)malloc(sizeof(int) * (nroots));
	pert = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);

	m = (double*)malloc(sizeof(double) * n);
	a = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
	prodevs = (double*)malloc(sizeof(double) * n);
	devs = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
	init = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (n + 1));
	centralimages = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nnm1) / 2);
	errs = (double*)malloc(sizeof(double) * nroots);
	newseeds = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (2 * nroots));
	grads = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	S2s = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	S3s = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
	S4s = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots));
...
void VBMicrolensing::change_n_mp(int nn) {
	...
	s_sort = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
	y_mp = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);

	m_mp = (double**)malloc(sizeof(double*) * n);
	a_mp = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	for (int i = 0; i < n; i++) {
		m_mp[i] = (double*)malloc(sizeof(double) * n);
		a_mp[i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
	}

	coefs_mp = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	pza_mp = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	ppy_mp = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	for (int j = 0; j < n; j++) {
		coefs_mp[j] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots + 1));
		pza_mp[j] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (n + 1));
		ppy_mp[j] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nroots + 1));
	}

	pmza_mp = (VBMcomplex***)malloc(sizeof(VBMcomplex**) * n);
	pyaza_mp = (VBMcomplex***)malloc(sizeof(VBMcomplex**) * n);
	ppmy_mp = (VBMcomplex***)malloc(sizeof(VBMcomplex**) * n);
	for (int j = 0; j < n; j++) {
		pmza_mp[j] = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
		pyaza_mp[j] = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
		ppmy_mp[j] = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
		for (int i = 0; i < n; i++) {
			pmza_mp[j][i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * n);
			pyaza_mp[j][i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (n + 1));
			ppmy_mp[j][i] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * (nnm1 + 1));
		}
	}
	dist_mp = (double*)malloc(sizeof(double) * n);

	zr_mp = (VBMcomplex**)malloc(sizeof(VBMcomplex*) * n);
	for (int j = 0; j < n; j++) {
		zr_mp[j] = (VBMcomplex*)malloc(sizeof(VBMcomplex) * nroots);
		for (int i = 0; i < n; i++) {
			zr_mp[j][i] = 0;
		}
	}
...
void VBMicrolensing::polycritcoefficients(VBMcomplex eiphi) {
	static int dg;
	static VBMcomplex pbin[3];
...
void VBMicrolensing::cmplx_roots_gen(VBMcomplex * roots, VBMcomplex * poly, int degree, bool polish_roots_after, bool use_roots_as_starting_points) {
...
void VBMicrolensing::cmplx_roots_multigen(VBMcomplex * roots, VBMcomplex * *poly, int degree, bool polish_roots_after, bool use_roots_as_starting_points) {

	static VBMcomplex poly2[MAXM];
	static int l, j, i, k, nl, ind, degreenew, croots, n;
	static double dif0, br;
	static bool success;
	static VBMcomplex coef, prev, przr;
...
void VBMicrolensing::solve_quadratic_eq(VBMcomplex & x0, VBMcomplex & x1, VBMcomplex * poly) {
	static VBMcomplex a, b, c, b2, delta;
	a = poly[2];
	b = poly[1];
	c = poly[0];
	b2 = b * b;
...
void VBMicrolensing::solve_cubic_eq(VBMcomplex & x0, VBMcomplex & x1, VBMcomplex & x2, VBMcomplex * poly) {
	...
	static VBMcomplex zeta = VBMcomplex(-0.5, 0.8660254037844386);
	static VBMcomplex zeta2 = VBMcomplex(-0.5, -0.8660254037844386);
	static double third = 0.3333333333333333;
	static VBMcomplex s0, s1, s2;
	static VBMcomplex E1; //x0+x1+x2
	static VBMcomplex E2; //x0*x1+x1*x2+x2*x0
	static VBMcomplex E3; //x0*x1*x2
	static VBMcomplex A, B, a_1, E12, delta, A2;

	static VBMcomplex val, x;
	a_1 = 1 / poly[3];
	E1 = -poly[2] * a_1;
	E2 = poly[1] * a_1;
	E3 = -poly[0] * a_1;
...
void VBMicrolensing::cmplx_laguerre(VBMcomplex * poly, int degree, VBMcomplex * root, int& iter, bool& success) {
	//Subroutine finds one root of a VBMcomplex polynomial using
	...
	static VBMcomplex p, dp, d2p_half; //value of polynomial, 1st derivative, and 2nd derivative
	static int i, j, k;
	static bool good_to_go;
	static VBMcomplex denom, denom_sqrt, dx, newroot;
	static double ek, absroot, abs2p;
	static VBMcomplex fac_newton, fac_extra, F_half, c_one_nth;
	static double one_nth, n_1_nth, two_n_div_n_1;
	static VBMcomplex c_one = VBMcomplex(1, 0);
	static VBMcomplex zero = VBMcomplex(0, 0);
	...
	c_one_nth = VBMcomplex(one_nth, 0.0);
	for (i = 1; i <= MAXIT; i++) {
		...
		if (denom == 0) {
			dx = (absroot + 1.0) * expcmplx(VBMcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI));
		}
		else {
			dx = fac_newton / denom;
		}
...
void VBMicrolensing::cmplx_newton_spec(VBMcomplex * poly, int degree, VBMcomplex * root, int& iter, bool& success) {
	//Subroutine finds one root of a VBMcomplex polynomial
	...
	static VBMcomplex p; //value of polynomial
	static VBMcomplex dp; //value of 1st derivative
	static int i, k;
	static bool good_to_go;
	static VBMcomplex dx, newroot;
	static double ek, absroot, abs2p;
	static VBMcomplex zero = VBMcomplex(0, 0);
	static double stopping_crit2;
	...
	for (i = 1; i <= MAXIT; i++) {
		...
		if (dp == zero) {
			//problem with zero
			dx = (abs(*root) + 1.0) * expcmplx(VBMcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI));
		}
		else {
...
void VBMicrolensing::cmplx_laguerre2newton(VBMcomplex * poly, int degree, VBMcomplex * root, int& iter, bool& success, int starting_mode) {
	//Subroutine finds one root of a VBMcomplex polynomial using
	...
	static VBMcomplex p; //value of polynomial
	static VBMcomplex dp; //value of 1st derivative
	static VBMcomplex d2p_half; //value of 2nd derivative
	static int i, j, k;
	static bool good_to_go;
	//VBMcomplex G, H, G2;
	static VBMcomplex denom, denom_sqrt, dx, newroot;
	static double ek, absroot, abs2p, abs2_F_half;
	static VBMcomplex fac_netwon, fac_extra, F_half, c_one_nth;
	static double one_nth, n_1_nth, two_n_div_n_1;
	static int mode;
	static VBMcomplex c_one = VBMcomplex(1, 0);
	static VBMcomplex zero = VBMcomplex(0, 0);
	...
	for (;;) { //infinite loop, just to be able to come back from newton, if more than 10 iteration there
		...
		if (mode >= 2) {//Laguerre's method
			...
			for (i = 1; i <= MAXIT; i++) {
				...
				if (denom == zero) {//test if demoninators are > 0.0 not to divide by zero
					dx = (abs(*root) + 1.0) + expcmplx(VBMcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI)); //make some random jump
				}
				else {
					dx = fac_netwon / denom;
				}
		...
		if (mode == 0) { // Newton's Method
			...
			for (i = j; i <= j + 10; i++) { // Do only 10 iterations the most then go back to Laguerre
				...
				if (dp == zero) {
					dx = (abs(*root) + 1.0) * expcmplx(VBMcomplex(0.0, 2 * M_PI * FRAC_JUMPS[i % FRAC_JUMP_LEN])); // make a random jump
				}
				else {
					dx = p / dp;
				}
