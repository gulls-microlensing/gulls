...
double abs(VBBcomplex);
VBBcomplex conj(VBBcomplex);
VBBcomplex sqrt(VBBcomplex);
double real(VBBcomplex);
double imag(VBBcomplex);
VBBcomplex operator+(VBBcomplex,VBBcomplex);
VBBcomplex operator-(VBBcomplex,VBBcomplex);
VBBcomplex operator*(VBBcomplex,VBBcomplex);
VBBcomplex operator/(VBBcomplex,VBBcomplex);
VBBcomplex operator+(VBBcomplex,double);
VBBcomplex operator-(VBBcomplex,double);
VBBcomplex operator*(VBBcomplex,double);
VBBcomplex operator/(VBBcomplex,double);
VBBcomplex operator+(double,VBBcomplex);
VBBcomplex operator-(double,VBBcomplex);
VBBcomplex operator*(double,VBBcomplex);
VBBcomplex operator/(double,VBBcomplex);
VBBcomplex operator+(int,VBBcomplex);
VBBcomplex operator-(int,VBBcomplex);
VBBcomplex operator*(int,VBBcomplex);
VBBcomplex operator/(int,VBBcomplex);
VBBcomplex operator+(VBBcomplex,int);
VBBcomplex operator-(VBBcomplex,int);
VBBcomplex operator*(VBBcomplex,int);
VBBcomplex operator/(VBBcomplex,int);
VBBcomplex operator-(VBBcomplex);
...
_sols *VBBinaryLensing::PlotCrit(double a1,double q1){
	...
	for(int j=0;j<NPS;j++){
		ej=VBBcomplex (cos(2*j*M_PI/NPS),-sin(2*j*M_PI/NPS));
		VBBcomplex  coefs[5]={a*a/16.0*(4.0-a*a*ej)*(1.0+q),a*(q-1.0),(q+1.0)*(1.0+a*a*ej/2.0),0.0,-(1.0+q)*ej};
...
// Caustics
	...
	for(Prov=CriticalCurves->last; Prov; Prov=Prov->prev){
		...
		for(_point *scanpoint=Prov->first; scanpoint; scanpoint=scanpoint->next){
			x1=VBBcomplex (scanpoint->x1-centeroffset,0.0);
			x2=VBBcomplex (scanpoint->x2,0.0);
...
_curve *VBBinaryLensing::NewImages(VBBcomplex yi,VBBcomplex  *coefs,_theta *theta){
	static VBBcomplex  y,yc,z,zc,zr[5]={0.,0.,0.,0.,0.},dy,dz,dJ;
	...
	if(good[worst1]>dlmax){
		for(int i=0;i<5;i++){
			if((i!=worst1)&&(i!=worst2)){
				...
				zc=VBBcomplex(Prov->last->x1, - Prov->last->x2);
				z=_J1c;
				dJ=1-z*conj(z);
				dy=VBBcomplex(-sin(theta->th),cos(theta->th))*coefs[23];
				dz=(dy-z*conj(dy))/dJ;
				z=conj(zc);
...
double VBBinaryLensing::BinaryMag0(double a1,double q1,double y1v,double y2v,_sols **Images){
	static VBBcomplex a,q,m1,m2,y;
	static double av=-1.0,qv=-1.0;
	static VBBcomplex  coefs[24],d1,d2,dy,dJ,dz;
	double Mag=-1.0;
	_theta *stheta;
	_curve *Prov, *Prov2;
	_point *scan1,*scan2;
...
double VBBinaryLensing::BinaryMag(double a1,double q1,double y1v,double y2v,double RSv,double Tol,_sols **Images){
	static VBBcomplex a,q,m1,m2,y0,y,yc,z,zc;
	static double av=-1.0,qv=-1.0;
	static VBBcomplex coefs[24],d1,d2,dy,dJ,dz;
...
// Calculation of the images
	...
	y=y0+VBBcomplex(RSv*cos(thoff),RSv*sin(thoff));
	...
	do{
		...
		//}
		stheta=Thetas->insert(th);
		y=y0+VBBcomplex(RSv*cos(th),RSv*sin(th));
#ifdef _PRINT_TIMES
		tim0=Environment::TickCount;
#endif
		Prov=NewImages(y,coefs,stheta);
...
VBBcomplex::VBBcomplex(double a,double b){
	re=a;
	im=b;
}

VBBcomplex::VBBcomplex(double a){
	re=a;
	im=0;
}

VBBcomplex::VBBcomplex(void){
	re=0;
	im=0;
}

double abs(VBBcomplex z){
	return sqrt(z.re*z.re+z.im*z.im);
}

VBBcomplex conj(VBBcomplex z){
	return VBBcomplex(z.re,-z.im);
}

VBBcomplex sqrt(VBBcomplex z){
	double md=sqrt(z.re*z.re+z.im*z.im);	
	return (md>0)? sqrt(md)*VBBcomplex((sqrt((1+z.re/md)/2)*((z.im>0) ? 1 : -1)),sqrt((1-z.re/md)/2)) : 0.0;
}

double real(VBBcomplex z){
	return z.re;
}

double imag(VBBcomplex z){
	return z.im;
}

VBBcomplex operator+(VBBcomplex p1,VBBcomplex p2){
	return VBBcomplex(p1.re+p2.re,p1.im+p2.im);
}
...
VBBcomplex operator*(VBBcomplex p1,VBBcomplex p2){
	return VBBcomplex(p1.re*p2.re-p1.im*p2.im,p1.re*p2.im+p1.im*p2.re);
}

VBBcomplex operator/(VBBcomplex p1,VBBcomplex p2){
	double md=p2.re*p2.re+p2.im*p2.im;
	return VBBcomplex((p1.re*p2.re+p1.im*p2.im)/md,(p1.im*p2.re-p1.re*p2.im)/md);
}

VBBcomplex operator+(VBBcomplex z,double a){
	return VBBcomplex(z.re+a,z.im);
}
...
VBBcomplex operator*(VBBcomplex z,double a){
	return VBBcomplex(z.re*a,z.im*a);
}
...
VBBcomplex operator+(double a,VBBcomplex z){
	return VBBcomplex(z.re+a,z.im);
}

VBBcomplex operator-(double a,VBBcomplex z){
	return VBBcomplex(a-z.re,-z.im);
}

VBBcomplex operator*(double a,VBBcomplex z){
	return VBBcomplex(a*z.re,a*z.im);
}

VBBcomplex operator/(double a,VBBcomplex z){
	double md=z.re*z.re+z.im*z.im;
	return VBBcomplex(a*z.re/md,-a*z.im/md);
}


VBBcomplex operator+(VBBcomplex z,int a){
	return VBBcomplex(z.re+a,z.im);
}
...
VBBcomplex operator+(int a,VBBcomplex z){
	return VBBcomplex(z.re+a,z.im);
}
...
VBBcomplex operator-(VBBcomplex z){
	return VBBcomplex(-z.re,-z.im);
}


void VBBinaryLensing::laguer(VBBcomplex *a, int m, VBBcomplex *x, int *its,double dzmax){
	static int iter,j;
	static double abx,abp,abm,err;
	static VBBcomplex dx,x1,b,d,f,g,h,sq,gp,gm,g2;
	...
	for(iter=1;iter<=MAXIT;iter++){
		*its=iter;
		...
		dx=(abp >0.0 ? (m/gp) :  VBBcomplex ((1+abx)*cos((double) iter),(1+abx)*sin((double) iter)));
		x1=*x - dx;
...
void VBBinaryLensing::zroots(VBBcomplex *a,int m,VBBcomplex *roots, int polish,double dzmax){
	static int its,j,jj;
	static VBBcomplex x,b,c,ad[MAXM];
